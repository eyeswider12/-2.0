...
# 兼容：Windows / macOS / Linux（便携版优先使用同目录 ffmpeg.exe）
...
# -------------------- 便携版路径配置（必须在最顶部）--------------------
def get_base_dir():
    """获取程序根目录：打包后指向 exe 所在文件夹，开发时指向脚本所在文件夹"""
...
os.environ["HF_HUB_OFFLINE"] = "1"   # 强制离线，避免联网下载

# 优先使用同目录的 ffmpeg.exe / ffprobe.exe（绿色版关键）
...
def burn_subtitles(video_path, srt_path, output_path):
    ...
    vf_filter = (
        ...
        f"force_style='FontName=SimHei,FontSize=24,PrimaryColour=&H00FFFFFF,OutlineColour=&H00000000,Outline=2':"
